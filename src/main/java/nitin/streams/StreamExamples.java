...
public class StreamExamples {
    public static void main(String[] args) {
        List<String> words = Arrays.asList("hi", "hello", "ho la", "bye", "goodbye", "adios");
        ...
        words.stream().filter(wordsWithSpace).forEach(s -> System.out.println("  " + s));

        // Problem 2
        System.out.println("Words (no spaces):");
        words.stream().filter(wordsWithSpace.negate()).forEach(System.out::println);
        ...
        List<String> wordsWithB =
                words.stream().filter(s -> s.contains("b")).collect(Collectors.toList());
        ...
        // Problem 7
        String[] excitingWords2 = words.stream().map(s -> s + "!").toArray(String[]::new);
