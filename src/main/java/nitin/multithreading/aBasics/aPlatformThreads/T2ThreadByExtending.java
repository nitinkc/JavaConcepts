 ...
 *
 * <p>Difference between t.start() and t.run() t.start calls run() from within. if t.run is
 * executed, run method will execute normally.
 *
 * <p>ALSO. Since by extending, we are limiting to extending only one class. NO CHANCE OF EXTENDING
 * ANY OTHER CLASS!! We cannot extend any other class. Thus implementing Runnable Interface is
