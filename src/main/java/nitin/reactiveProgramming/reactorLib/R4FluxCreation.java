...
public class R4FluxCreation {
    public static void main(String[] args) {
        ...
        stringFlux.subscribe(ReactorUtils.onNext(),
                ReactorUtils.onError(),
                ReactorUtils.onComplete()
        );

        //Multi Element Flux
        Flux<String> multiStringFlux = Flux.just(
        ...
        //Flux from ArrayList
        List<String> titles = List.of(ReactorUtils.faker().book().title(),
                ReactorUtils.faker().book().title(),
                ReactorUtils.faker().book().title());
        ...
        Flux<String> rangeNames = Flux
                .range(6, 5)
                .log()
                .map(iterator -> ReactorUtils.faker().book().title())
                .log();
