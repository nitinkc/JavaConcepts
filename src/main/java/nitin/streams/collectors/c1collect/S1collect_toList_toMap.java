...
public class S1collect_toList_toMap {
    public static void main(String[] args) {
        ...
        List<String> l = list.stream().filter(str -> str.length() >= 9).toList();
        ...
        List<String> l1 = list.stream().filter(strShort).toList();
        ...
        Map<String, Integer> map =
                list.stream().collect(Collectors.toMap(identity(), String::length));
        ...
        Map<Integer, List<String>> collect =
                list.stream()
                        .collect(
                                groupingBy(
                                        str -> str.length(),
                                        mapping(str -> str.toUpperCase(), toList())));
