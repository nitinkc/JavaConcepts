...
public class MapEmployees {
    public static void main(String[] args) {
        ...
        List<Integer> ids = employees.stream().map(Employee::getId).collect(toList());

        int totalLength =
                employees.stream()
                        .map(Employee::getName)
                        .mapToInt(String::length) // mapToInt has additional methods that can only
                        // be applied to integers, like sum, average etc
                        .sum();
        ...
        Map<Integer, Employee> employeeMap =
                employees.stream()
                        .collect(
                                toMap(
                                        Employee::getId,
                                        Function.identity())); // Function.identity() means e -> e
