...
public class FileReadingExamples {
    ...
    /**
     * The enable1 word list is a public-domain file containing over 175,000 supposed words accepted
     * by many US Scrabble clubs. The name comes from Enhanced North American Benchmark LExicon
     * (ENABLE). It is almost twice as large as the Official Scrabble Player's Dictionary, and
     * contains slang, offensive words, and many obscure or questionable words. It contains no
     * one-letter words and no super-long words, and is not endorsed in any way by Hasbro (maker of
     * Scrabble) or Merriam Webster (publisher of The Official Scrabble Player's Dictionary).
     * Details at http://www.puzzlers.org/dokuwiki/doku.php?id=solving:wordlists:about:enable_readme
     *
     * <p>Two repeated words in mixed case (Extra and EXTRA) were added to the end of the file to
     * verify that the code can handle repeats, mixed case, and out-of-order entries.
     *
    ...
    public static void testLetterCount(String filename) {
        ...
        int sum3 =
                FileUtils.letterCount(
                        filename, word -> word.contains("q"), word -> !word.contains("qu"));
    ...
    public static void testFirstMatch(String filename) {
        ...
        Integer match1 =
                FileUtils.firstMatch(testNums.stream(), n -> n > 2, n -> n < 10, n -> n % 2 == 1);
        System.out.printf(
                "First word in list %s that is greater than 2, less than 10, and odd is %s.%n",
                testNums, match1);
        String match2 =
                FileUtils.firstMatch(
                        filename, word -> word.contains("q"), word -> !word.contains("qu"));
        System.out.printf(
                "First word in file %s with q not followed by u is %s.%n", filename, match2);
    }

    public static void testAllMatches(String filename) {
        List<Integer> testNums = Arrays.asList(2, 4, 6, 8, 10, 12);
        List<Integer> matches1 = FileUtils.allMatches(testNums.stream(), n -> n > 5, n -> n < 10);
        System.out.printf(
                "All numbers in list %s that are greater than 5 and less than 10: %s.%n",
                testNums, matches1);
        List<String> matches2 =
                FileUtils.allMatches(
                        filename, word -> word.contains("q"), word -> !word.contains("qu"));
        System.out.printf(
                "All words in file %s with q not followed by u: %s.%n", filename, matches2);
    }
}
