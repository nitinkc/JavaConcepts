...
public class ECallMultipleService {
    ...
    */
    public static void main(String[] args) {
        ...
        // 1. Gutendex search (Search the Author from book name) -
        ...
        for (int i = 0; i < itemList.size(); i++) {
            ...
            isbnList.addAll(
                    item.getIsbn().stream().filter(x -> x != null).collect(Collectors.toList()));
        }
        // String isbn = itemList.stream().findFirst().get().getIsbn().stream().findFirst().get();

        // 3. Find Book Details from ISBN -
        // https://openlibrary.org/api/books?bibkeys=ISBN:9781108074568&format=json&jscmd=data
        String isbn =
                isbnList.stream()
                        .reduce("", (partialString, element) -> partialString + element + ",");

        List<OpenLibDto> openLibDto =
                RestGETReadUtility.getBookDetailsOpenLibrary(isbn).stream()
