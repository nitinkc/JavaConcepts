...
/**
 * Finds various information about the word list. Uses the third and fourth file-reading variations,
 * where the code is split into a file-processing method and a stream-processing method, and code in
 * the file-processing method is NOT repeated from example to example.
 */
public class FileUtils {
    ...
    /** Returns the first palindrome in the Stream. Returns null if there is no match. */
    public static String firstPalindrome(Stream<String> words) {
        return (words.filter(StringUtils::isPalindrome).findFirst().orElse(null));
    }

    // @SafeVarargs is difficult to understand. The issue is that it is not always safe to use
    // varargs for generic types:
    ...
    /** Returns the first palindrome in the file. Returns null if there is no match. */
    public static String firstPalindrome(String filename) {
        return (StreamAnalyzer.analyzeFile(filename, FileUtils::firstPalindrome));
    }

    /**
     * Returns a Predicate that is the result of ANDing all the argument Predicates. If no
     * Predicates are supplied, it returns a Predicate that always returns true.
     */
    @SafeVarargs
    ...
    /**
     * Returns first element in Stream that passes all of the tests. Returns null if there is no
     * match.
     */
    @SafeVarargs
    ...
    /**
     * Returns first line in file that passes all of the tests. Returns null if there is no match.
     */
    @SafeVarargs
    ...
    public static <T> List<T> allMatches(Stream<T> elements, Predicate<T>... tests) {
        Predicate<T> combinedTest = FileUtils.combinedPredicate(tests);
        return (elements.filter(combinedTest).collect(Collectors.toList()));
    }
    ...
    /**
     * Returns sum of the lengths of all words in the Stream that pass the tests. Returns 0 in no
     * words pass all the tests.
     */
    @SafeVarargs
    ...
    /**
     * Returns sum of the lengths of all lines in the file that pass the tests. Returns 0 in no
     * lines pass all the tests.
     */
    @SafeVarargs
