...
public class Intro {
    public static void main(String[] args) {
        ...
        Map<Boolean, List<Integer>> collect =
                list.stream().collect(partitioningBy(number -> number % 2 == 0));
        System.out.println(
                collect); // {false=[1, 1, 3, 3, 5, 7, 5, 3, 1], true=[2, 4, 6, 8, 6, 4, 2]}
    ...
    private static void splitEmpList(List<EmployeeSimple> employees) {
        ...
        Map<Boolean, List<EmployeeSimple>> listMap =
                employees.stream()
                        ...
                        .collect(
                                partitioningBy(
                                        evenAgedEmpPredicate)); // ⌘Cmd ⌥Opt V	Declare the variable

        System.out.println(listMap);
    }

    private static void doNotDo(List<EmployeeSimple> employees) {
        List<EmployeeSimple> evenAgedEmp =
                employees.stream()
        ...
        List<EmployeeSimple> oddAgedEmp =
                employees.stream()
                        .filter(Objects::isNull)
                        .filter(emp -> null != emp.getAge())
                        .filter(employee -> employee.getAge() % 2 != 0)
