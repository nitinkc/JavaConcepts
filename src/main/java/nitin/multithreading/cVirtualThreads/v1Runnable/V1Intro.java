...
public class V1Intro {
    static final int MAX_THREADS = 100_000; // One Million
    ...
    private static void platformThreads() throws InterruptedException {
        ...
        for (Thread thread : threadList) {
            thread.join(); // Joining all of the threads so that the next set of instructions does
            // not run until this is done.
        }
    }
    ...
    private static Thread createPlatformThread() {
        // The Virtual thread starts as a Daemon thread where as the platform thread starts as a
        // non-daemon thread
        return new Thread(
                Business::executeBusinessLogic); // JVM Can't handle large number threads with
        // platform threads
    }

    private static Thread createVirtualThread() {
        // With Virtual threads, mounting and unmounting would happen so a thread is free to do
        // tasks
