...
public class IntersectionSync implements Intersection {
    ...
    public void takeNorthSouthTrack() {
        synchronized (northSouthLock) {
            System.out.println(
                    "NorthSouthTrack is locked by thread " + Thread.currentThread().getName());
    ...
    public void takeEastWestTrack() {
        synchronized (northSouthLock) { // This will result in a deadlock. To avoid, keep the same
            // sequence of Locks everywhere
            System.out.println(
                    "EastWestTrack is locked by thread " + Thread.currentThread().getName());
            synchronized (eastWestLock) {
