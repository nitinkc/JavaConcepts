...
public class A1Intro {
    ...
    private static void eg1() {
        ...
        CompletableFuture.supplyAsync(
                        () -> { // Runs in a separate thread and releases it
                            System.out.println("supplier: " + Thread.currentThread());
                            return Math.PI;
                        })
                .thenAccept(
                        data -> System.out.println("Message received from supply Async: " + data));

        System.out.println("main thread2: " + Thread.currentThread());
    }

    private static void eg2() {
        CompletableFuture<Void> thisNeverEnds =
                getData()
    ...
    private static void eg3() {
        try {
            System.out.println(
                    getData().get()); // BAD IDEA, forces exception handling, it's a BLOCKING CALL
            // for forGET
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (ExecutionException e) {
    ...
    public static CompletableFuture<Double> getData() {
        return CompletableFuture.supplyAsync(
                () -> {
                    ...
                    MultiThreadUtility.delay(
                            1000); // Try commenting and see how it's changing the thread
