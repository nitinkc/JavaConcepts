...
/**
 * @author Created by nichaurasia Created on Monday, September/28/2020 at 12:07 AM
 */
public class EmployeeServices {
    ...
    private static void avgSalariesSum(List<EmployeeSimple> list) {
        OptionalDouble avgSalOptional =
                list.stream().mapToDouble(EmployeeSimple::getSalary).average();

        avgSalOptional.ifPresent(System.out::println);
    }

    private static void avgSalariesreduce(List<EmployeeSimple> list) {
        System.out.println(list.stream().map((e) -> e.getSalary()).reduce(0.0, (x, y) -> (x + y)));
    }

    private static void howManyGT30Employee(List<EmployeeSimple> list) {

        System.out.println(list.stream().filter(Lambdas.ageGT30Predicate).count());
    }
}
