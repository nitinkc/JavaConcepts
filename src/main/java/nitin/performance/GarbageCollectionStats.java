...
public class GarbageCollectionStats {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println(
                "Available memory  when customers created: "
                        + runtime.freeMemory() / bytesInMb
                        + unit);
        customers = new ArrayList<>();
        System.out.println(
                "Available memory  when customers no longer referenced: "
                        + runtime.freeMemory() / bytesInMb
                        + unit);
        Thread.sleep(1000);
        System.out.println(
                "Available memory  1 second later: " + runtime.freeMemory() / bytesInMb + unit);
        System.gc();
        System.out.println(
                "Available memory  after GC command post Java 11 Optimizations: "
                        + runtime.freeMemory() / bytesInMb
                        + unit);
    }
}
