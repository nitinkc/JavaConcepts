...
public class C1Introduction {
	...
	private static void commaSeparatedList() {
		...
		//Off-by-one error
		String commaSeparatedNamesWithExtra = new String();
		for (EmployeeSimple employeeSimple : employees){
			commaSeparatedNamesWithExtra = commaSeparatedNamesWithExtra + employeeSimple.getName() + " ,";
		}
		...
		String commaSeparatedNames = new String();
		commaSeparatedNames = employees.stream()
	...
	private static void unmodifiableList() {
		...
		ages = employees.stream()
				.filter(Objects::nonNull).filter(employee -> null != employee.getAge())
				.map(employeeSimple -> employeeSimple.getAge())
				//.collect(Collectors.toList());
				.collect(Collectors.toUnmodifiableList());

		//Boom
		ages.add(999);//mutability with regular collection, can be avoided by using unmodifiable list
	...
	private static void creatingMap() {
		...
		nameAgeMap = employees.stream().parallel()
				.filter(employee -> null != employee.getAge())
				//.collect(Collectors.toMap(keyFunction, valueFunction));
				//.collect(Collectors.toMap(employee -> employee.getName(),employee -> employee.getAge()))
	...
	private static void usingCollector() {
		...
		youngEmployees = employees.stream()
				...
				.collect(Collectors.toList());//ThreadSafe and can handle concurrency easily
	...
	private static void sharedMutabilityReduce() {
		...
		youngEmployees = employees.stream().parallel()
				.filter(Objects::isNull)
				.filter(employee -> employee.getAge() < 25)
				.map(EmployeeSimple::getName)//get the name
				.map(String::toUpperCase)//convert to upper case
				.reduce(//Can work with parallel streams
						new ArrayList<String>(),//Local mutability, internal mutability, no outside arrayList is used
