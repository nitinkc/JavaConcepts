 ...
 *
 * <p>While reading a single value of a file input stream instance, the read method returns a
 * primitive int value rather than a byte value. If the class does return a byte instead of an int,
 * then there no is no way to know EOF. For compatibility, the file output stream also uses int
