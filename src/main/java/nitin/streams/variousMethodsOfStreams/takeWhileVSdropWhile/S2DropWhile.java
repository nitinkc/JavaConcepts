...
/**
 * Created by nitin on Sunday, January/26/2020 at 8:21 PM TakeWhile runs until the first
 * false/negative condition is met DropWhile runs from the first negative condition is met.
 */
        ...
        List<String> takeWhileList =
                list.stream()
                        ...
                        .takeWhile(
                                str ->
                                        str.length()
                                                < 7) // take the strings until a string of lenght 7
                        // comes
                        .collect(Collectors.toList());
                ...
                Collections.singletonList(
                        Optional.of(
                                        list.stream()
                                                .filter(
                                                        singleStr ->
                                                                null != singleStr) // Removing nulls
                                                .sorted(Comparator.naturalOrder())
                                                .dropWhile(str -> str.isBlank()) // Removing
                                                // Empty Strings
