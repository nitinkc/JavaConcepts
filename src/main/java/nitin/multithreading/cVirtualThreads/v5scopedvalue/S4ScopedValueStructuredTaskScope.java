...
public class S4ScopedValueStructuredTaskScope {
    ...
    public static void main(String[] args) throws Exception {
        ScopedValue.where(studentScopedValue, new Student("Harry Potter"))
    ...
    private static String invokeTaskScope() throws InterruptedException {
        ...
        try (var scope = new StructuredTaskScope<String>("child-scope", factory)) { //
            ...
            scope.join();
        } // All child threads finish in the structured task scope. Child thread can't run beyond
        // Parent thread

        Student reqUser =
                studentScopedValue.orElse(
                        new Student("Ron Weasley")); // Set new if there is nothing returned
        logShortMessage("invokeTaskScope - user " + reqUser);
        return "done";
    }
}
