...
public class R3ReflectionProbeMethods {
    ...
    private static void printClassMetadata(Class c) {
        ...
        for (Method method : m) {
            System.out.println(
                    "  " + method + " | declared in: " + method.getDeclaringClass().getName());
        }
        System.out.println("---------------------------");

        Method[] methods = c.getMethods();
        System.out.println(
                "METHODS IN CLASS ARE (getMethods - public only, includes superclasses):");
        if (methods.length == 0) {
        ...
        for (Field field : f) {
            System.out.println(
                    "  " + field + " | declared in: " + field.getDeclaringClass().getName());
        }
        System.out.println("---------------------------");
