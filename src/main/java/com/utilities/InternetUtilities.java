...
public class InternetUtilities {
    ...
    private static final String DATA_MUSE_WORDS_URL =
    ...
    public static List<String> getWords(String[] args) {
        ...
        ResponseEntity<WordResponse[]> response =
                restTemplate.getForEntity(DATA_MUSE_WORDS_URL, WordResponse[].class, uriVariables);
    ...
    public static List<Beer> getBeers(int size) {
        ...
        try {
            beerList =
                    mapper.readValue(
                            new URL(RANDOM_BEERS_URL + size), new TypeReference<List<Beer>>() {});
        } catch (IOException e) {
            e.printStackTrace();
        }
        return beerList;
