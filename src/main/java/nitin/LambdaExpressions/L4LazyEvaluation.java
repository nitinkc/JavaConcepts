 ...
 *
 * <p>The double of the first even number > 3 and in the a_list
 */
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                values.stream()
                        .filter(
                                L4LazyEvaluation
                                        ::isGT3) // another way of calling a method from lambda
                        // .filter(value -> L4LazyEvaluation.isGT3(value))
                        .filter(
                                L4LazyEvaluation
                                        ::isEven) // functions like filter and map are called
                        // intermediate functions.  They are LAzy
                        .map(L4LazyEvaluation::doubleIt)
                        .findFirst() // terminal Function: triggers the computations, the code
                        // ACTUALLY STARTS WORKING
                        // FROM THIS POINT ON due to LAziness. The computations are only enough to
                        // get the work done
                        .orElse(0));

        // Demonstrating the LAZY way by removing the terminal function. the intermediatry doent
        // even do anything
        ...
        values.stream()
                ...
                .filter(
                        L4LazyEvaluation
                                ::isEven) // functions like filter and map are called intermediate
                // functions.  They are LAzy
                .map(L4LazyEvaluation::doubleIt);
