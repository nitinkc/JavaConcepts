...
public class A10ASucceedOnTimeOut {
    public static void main(String[] args) {
        ...
        // does not keep the pipeline in PENDING STATE more than a second. If the values doesnt
        // arrive in a second,
        // then resolve it with the default value being passes

        delay(2000);

        // If there is delay for complete to run more than time out value from completeOnTimeout,
        // then complete on time
        // out code will run
    ...
    private static void successOnTimeOut(CompletableFuture<Integer> future) {
        future.completeOnTimeout(
                5, 1, TimeUnit.SECONDS); // Does not keep the pipeline in PENDING state
        // for more than a second. the value doesn't arrive in 1 sec (timeout) then resolve it, via
        // the default value
    }
    ...
    private static void failureOnTimeOut(CompletableFuture<Integer> future) {
        ...
        // for more than a second. the value doesn't arrive in 1 sec (timeout) then cancel it, and
        // completes it exceptionally with a TimeoutException
    }
}
