...
public class A3Pipeline {
    ...
    private static void establishAndrunPipeline() {
                ...
                completableFuture
                        ...
                        .thenApply(
                                x ->
                                        Double.parseDouble(
                                                String.valueOf(
                                                        x))) // If there is an exception, handle
                        // with exceptionally
                        .thenApply(x -> 2 / x)
                        .exceptionally(
                                throwable -> {
                        ...
                        .thenAccept(
                                x ->
                                        System.out.println(
                                                "rtr " + x)) // Behaves like ForEach, but not a
                        // reduction operation.
        ...
        voidCompletableFuture
                .join(); // Holding the main thread from quitting before spitting out the logs
        ...
        CompletableFuture<Void> voidCompletableFuture =
                completableFuture
                        ...
                        .thenAccept(
                                data ->
                                        System.out.println(
                                                data)) // forEach, like reduction operation
                        .thenRun(() -> System.out.println("Job Done!!")); // Still continue with the
        // CompletableFuture
