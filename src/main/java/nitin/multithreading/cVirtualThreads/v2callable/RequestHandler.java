package nitin.multithreading.cVirtualThreads.v2callable;
...
public class RequestHandler implements Callable<String> {
    ...
    private String concurrentCallCompletableFuture() {
        try (ExecutorService service = Executors.newVirtualThreadPerTaskExecutor()) {
            ...
            CompletableFuture<String> dbCallFuture =
                    CompletableFuture.supplyAsync(() -> Business.blockingDbCall(2), service)
                            .exceptionally(
                                    ex -> {
                                        System.err.println(
                                                "Exception occurred in dbCall: " + ex.getMessage());
            ...
            CompletableFuture<String> getBrewerFuture =
                    CompletableFuture.supplyAsync(() -> Business.getBrewer(), service)
                            .exceptionally(
                                    ex -> {
                                        System.err.println(
                                                "Exception occurred in getBrewer: "
                                                        + ex.getMessage());
            ...
            CompletableFuture<String> getBeerFuture =
                    CompletableFuture.supplyAsync(() -> Business.getBeer(), service)
                            .exceptionally(
                                    ex -> {
                                        System.err.println(
                                                "Exception occurred in getBeer: "
                                                        + ex.getMessage());
            ...
            String output =
                    CompletableFuture.allOf(dbCallFuture, getBrewerFuture, getBeerFuture)
                            .thenApplyAsync(
                                    voidResult -> {
    ...
    private String concurrentCallWithFutures() throws Exception {
        ...
        try (ExecutorService service = Executors.newVirtualThreadPerTaskExecutor()) {
            ...
            String result =
                    String.format(
                            "[%s,%s,%s]",
                            "concurrentCallWithFutures ::  ", dbFuture.get(), restFuture.get());
        ...
        try (ExecutorService service = Executors.newVirtualThreadPerTaskExecutor()) {
            ...
            String result =
                    service
                            .invokeAll(
                                    Arrays.asList(
                                            () -> Business.blockingDbCall(2), Business::getBeer))
                            .stream()
                            .map(
                                    f -> {
                                        try {
