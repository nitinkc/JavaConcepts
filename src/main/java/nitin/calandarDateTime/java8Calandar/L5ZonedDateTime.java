...
public class L5ZonedDateTime {
    public static void main(String[] args) {
        ...
        if (null != timeInTheCityWhereCodeIsExecuted && timeInTheCityWhereCodeIsExecuted.isAfter(timeInIndia)) {
            System.out.println("...1...EXCEPTION Start date should not less than Stop date");
        }
        if (timeInTheCityWhereCodeIsExecuted.toLocalDate().isBefore(timeInIndia.toLocalDate())) {//Considering the Date part only
            System.out.println("...2...Start date IS BEFORE Stop date");
        }
        if (timeInTheCityWhereCodeIsExecuted.toLocalDate().isEqual(timeInIndia.toLocalDate())) {//Considering the Date part only
