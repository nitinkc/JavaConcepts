...
public class StructuredTaskScopeRunner {
    ...
    private static void completeAllTasks() throws InterruptedException {
        var tasks = List.of(new BlockingIOTasks("task1", 3,false),
        ...
        try(var scope = new StructuredTaskScope<BlockingIOTasks.TaskResponse>()) {
            ...
            List<Subtask<BlockingIOTasks.TaskResponse>> subtasks = tasks.stream().map(task -> scope.fork(task)).toList();
    ...
    private static void shutdownOnFailure()
        ...
        var tasks = List.of(new BlockingIOTasks("Task#1", 3,true),
    ...
    private static void shutdownOnSuccess()
                throws InterruptedException, ExecutionException {
        var tasks = List.of(new BlockingIOTasks("Price-1", 3, true),
                new BlockingIOTasks("Price-2", 10,true));

        try(var scope = new StructuredTaskScope.ShutdownOnSuccess<BlockingIOTasks.TaskResponse>()) {
            // Start running the tasks in parallel
            List<Subtask<BlockingIOTasks.TaskResponse>> list = tasks.stream().map(task -> scope.fork(task)).toList();
