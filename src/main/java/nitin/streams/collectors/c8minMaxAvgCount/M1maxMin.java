...
public class M1maxMin {
    ...
    private static void minByMaxByIntro(List<EmployeeSimple> employees) {
        EmployeeSimple collect =
                employees.stream()
                        .filter(Objects::nonNull)
                        .filter(emp -> null != emp.getAge())
                        .filter(emp -> null != emp.getName())
                        .collect(Collectors.maxBy(Comparator.comparing(EmployeeSimple::getAge)))
    ...
    private static void minByAndMaxBy(List<EmployeeSimple> employees) {
        ...
        EmployeeSimple maxBy =
                employees.stream()
                        ...
                        .orElse(new EmployeeSimple());
        System.out.println(
                maxBy); // EmployeeSimple(name=Jane, age=35, salary=76546.0, level=B, experience=5)

        EmployeeSimple minBy =
                employees.stream()
                        .filter(Objects::nonNull)
                        .filter(emp -> null != emp.getName())
                        .filter(emp -> null != emp.getAge())
                        ...
                        .orElse(new EmployeeSimple());
        System.out.println(
                minBy); // EmployeeSimple(name=John, age=20, salary=65000.0, level=C, experience=5)
    }
}
